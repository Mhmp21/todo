    ...
    <div class="text-center flex flex-col justify-center w-1/3 mx-auto">
        ...
        <div class="rounded-md w-full bg-red-50 border-2 border-sky-200">
            <textarea id="title" name="description"
                class="form-control @error('description') appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white @enderror">{{ $todo->description }}</textarea>
        </div>
        @error('description')
            <div class="rounded-md flex-col w-12/12 bg-red-500 mt-3 mb-4 border-2 ">
                {{ $message }}
